...
class Trainer():
    def __init__(self, representation, lr=1e-3, print_freq=1):
        ...
        # freeze mlp_parameters
        # self.parameters_to_freeze = []
        # for param in self.representation.net.parameters():
        #     self.parameters_to_freeze.append(param)
        # freeze feature_parameters
        # for param in self.representation.features.parameters():
        #     self.parameters_to_freeze.append(param)
        self.optimizer = torch.optim.Adam([
            {'params': self.representation.net.parameters(),
             'lr': lr},
            {'params': self.representation.deepJSCC.parameters(),
             'lr': 2e-4},
            {'params': self.representation.log_gains,
             'lr': 2e-4},
            {'params': self.representation.features.parameters(),
             'lr': 0.01,
             'requires_grad': True}
        ])
    ...
    def train(self, coordinates, img, args, num_iters):
        ...
        with tqdm.trange(num_iters, ncols=100) as t:
            for i in t:
                ...
                predicted = self.representation.encode(coordinates, 0)
                predicted = predicted.permute(2, 0, 1)
                img = img.squeeze()
                loss = self.loss_func(predicted, img)
                if i >= 5000:
                    # for param in self.parameters_to_freeze:
                    #     param.requires_grad = False
                    jscc_distortion = self.representation.pass_channel()
                    loss = loss + jscc_distortion * 10
                loss.backward(retain_graph=True)
                # print(self.representation.features[0].grad)
                self.optimizer.step()
